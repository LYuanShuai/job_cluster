...
import PasswordEvent, {PasswordUpdateParam} from "../../dao/PasswordDao";

enum PasswordPageEvent {
    HIDE_CREATE_MODAL = "PasswordPage.hideCreateModal"
}
...
const PasswordPage: React.FC = () => {
    ...
    const [submitEvent, setSubmitEvent] = useState<string>(PasswordEvent.UPDATE)
    ...
    const [createModal, setCreateModal] = useState<CreateModalProps>({
        ...
        handleClose: () => ee.emit(PasswordPageEvent.HIDE_CREATE_MODAL),
        handleShow: () => {
            setCmShow(true)
        }
    })
    ...
    const showUpdateModal = (resource: Resource) => {
        ...
        setSubmitEvent(PasswordEvent.UPDATE)
        setCmShow(true)
    }
    ...
    const showFsm = (resource: Resource) => {
        ...
        setSubmitEvent(PasswordEvent.UPDATE_SCRIPT)
        setFsmShow(true)
    }
    ...
    useEffect(() => {
            updateAll()
            ee.on(PasswordPageEvent.HIDE_CREATE_MODAL, () => {
    ...
    const handleUpdate: (resource: any) => void = (resource: PasswordUpdateParam) => {
        ee.emit(submitEvent, resource, updateAll)
    }

    const handleCmSubmit: (resource: any) => void = (resource: PasswordUpdateParam) => {
    ...
    const handleFsmSubmit: (resource: any) => void = (resource: PasswordUpdateParam) => {
        handleUpdate(resource)
        handleFsmClose()
    }
